...
class ResearchProfilesTests(unittest.TestCase):
    ...
    def test_find_horizon_prediction_files_accepts_labeled_prediction_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            for name in (
            ...
            files = find_horizon_prediction_files(root, label_horizon_hours=72, year=2024)
